    ...
    <div class="grid grid-cols-3 gap-12">
        @foreach($posts as $post)
            <div class="group relative pb-40 overflow-hidden">
                ...
                <div class="absolute top-0 left-0 ml-8 mt-8">
                    ...
                    <div class="relative text-white bg-brand-crimson leading-none text-center p-4">
                        <p class="text-5xl font-extrabold">{{ $post->publish_at->format('d') }}</p>
                        <p class="mt-1 text-sm uppercase font-extrabold">{{ $post->publish_at->format('F') }}</p>
                    </div>
                </div>
                <div class="absolute bottom-0 left-0 mx-8 px-8 pt-8 bg-gray-50 -mt-16 -mb-20 transition-all duration-300 group-hover:mb-0">
                    <a
